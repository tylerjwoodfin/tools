...
Example Crontab:
0 1 * * * cd <bedtime.py folder> && XAUTHORITY=/home/<you>/.Xauthority DISPLAY=:0 python3 bedtime.py
"""
...
class App:
    ...
    def __init__(self, master, message) -> None:
        ...
        # block alt-tab
        print(os.system(
            "GSETTINGS_BACKEND=dconf /usr/bin/gsettings set org.gnome.desktop.wm.keybindings "
            "switch-applications \"[]\""
        ))
        ...
        while self.timeremaining > 0:
            self.text.set((f"{message}\nThis message will remain for "
                           f"{self.timeremaining} more "
                           f"{'second' if self.timeremaining == 1 else 'seconds'}."))
            root.update()
...
if __name__ == "__main__":
    CMD_ALT_TAB = "GSETTINGS_BACKEND=dconf /usr/bin/gsettings set "
    ...
    if len(sys.argv) > 1:
        ...
        MESSAGE = "Medicine and Melatonin!"
