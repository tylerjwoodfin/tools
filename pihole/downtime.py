...
class PiHoleDowntime:
    ...
    def should_block(self) -> bool:
        ...
        if current_hour >= 20:
            ...
            if self.is_holiday(tomorrow):
                self.cabinet.log("After 8PM and tomorrow is a holiday, skipping blocking")
                return False
            # After 8PM: if tomorrow is not a holiday, proceed with blocking
            self.cabinet.log("After 8PM and tomorrow is not a holiday, proceeding with blocking")
            return True
        else:
    ...
    def get_blocklist_file(self, mode: str) -> Optional[Path]:
        ...
        try:
            ...
            self.cabinet.log(f"Error getting blocklist file for mode '{mode}': {e}", level="error")
            return None
    ...
    def read_blocklist_domains(self, blocklist_file: Path) -> List[str]:
        ...
        try:
            if not blocklist_file.exists():
                self.cabinet.log(f"Blocklist file not found: {blocklist_file}", level="error")
                return []
    ...
    def cleanup_scheduled_jobs(self, mode: str):
        ...
        try:
            ...
            for line in result.stdout.strip().split('\n'):
                ...
                if (str(script_path) in job_content.stdout and 
                    "block" in job_content.stdout and 
                    mode in job_content.stdout):
    ...
    def run(self, action: str, mode: str):
        ...
        # Check parent process (except for block action)
        if action != "block" and not self.check_parent_process():
            print("This script can only be run from a Python script or crontab.")
            # Don't return, just continue with a warning (like the shell script)
